...
const state: IInitialStateState = {
    ...
    hall: null,
    cityList: typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('cityList') || '[]') : [],
    totalAmount: 0,
    sector: null,
    activeTab: typeof window !== 'undefined' ? Number(localStorage.getItem('tab')) || 1 : 1,
    sectorLegend: [],
...
export const mainReducer = createSlice({
    ...
    reducers: {
        ...
        clearOrderInfo: (state) => {
            const isTheSectorStandingPlace = state.sector?.scheme === 'typeIdOne';
            localStorage.removeItem('OrderData');
            state.selectedSession = null;
            state.selectedDay = null;
            state.currentOrderData = null;
            state.event = null;
            if (isTheSectorStandingPlace) {
                state.tickets.map((ticket) => (ticket.ticketCount = 0));
            } else {
                state.tickets = [];
            }
        },
