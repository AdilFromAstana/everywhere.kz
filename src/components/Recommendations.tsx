...
const Recommendations = ({ recs, UserLang }: RecommendationsProps) => {
    if (isEmpty(recs) || recs.length === 0) {
        return <></>;
    } else {
        return (
            <>
                <div className="lg:hidden relative flex flex-col items-center gap-4 py-10 -mx-4">
                    <svg
                        className="lg:hidden absolute -z-10 scale-105 w-[100vw] max-h-full left-0"
                        width="375"
                        height="353"
                        viewBox="0 0 375 353"
                        fill="none"
                    ...
                    >
                        <path
                            d="M-18 281C-7.32454 182.548 0.041666 -23.0023 27.0265 70.5745C86.1861 275.725 117.536 431.483 146.5 205.5C172.763 0.589828 113.772 31.2029 160.5 238.5C244.963 613.198 266.668 -110.764 283.5 21.9996C309.244 225.051 310.763 331.911 342.104 250.905C415 62.4995 359.211 49.9995 355.5 49.9995C339.22 49.9995 339.993 228.86 406.792 293.728"
                            stroke="#B9E4FF"
                            strokeWidth="10"
                        />
                    </svg>
                    ...
                    <div className="flex gap-4 items-center">
                        ...
                        <span className="font-semibold">Рекомендуем Вам</span>
                    </div>
                    <div className="flex gap-4 items-start overflow-x-auto w-full lg:px-0 px-4">
                        {recs.map((x, i: number) => (
                            <Link className="min-w-[40%] max-w-[40%]" href={x.URL} key={i}>
                                <div className="flex flex-col gap-2">
                                    <Image
                                        ...
                                        src={x.ImageURL}
                                    />
                                    <h6 className="text-[#2F2F38D9] font-semibold">{x.NameRu}</h6>
                                </div>
                            </Link>
                        ))}
                    </div>
                ...
                <div className="lg:block hidden">
                    <RecommendationsDesktop recs={recs} UserLang={UserLang} />
                </div>
            </>
        );
    }
