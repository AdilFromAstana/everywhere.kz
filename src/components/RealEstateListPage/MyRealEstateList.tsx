...
import MyRealEstateItem from './MyRealEstateItem';
import { IMyRealEstate } from '@/interfaces/RealEstate';

const MyRealEstateList: React.FC<any> = ({ locale, lookingFor }) => {
    const [realEstates, setRealEstates] = useState<IMyRealEstate[]>([]);
    const [selectedRealEstate, setSelectedRealEstate] = useState<any>({});
    ...
    const getButtons = (realEstate: any) => {
    ...
    const handleFilterToggle = (realEstate: any) => {
    ...
    const createStatusArray = (realEstateList: any[]) => {
        let statusList: any[] = [];
        realEstateList.forEach((realEstate, i) => {
            if (i === 0) setSelectedStatusCode(realEstate.RealEstateStatus.code);
            const status = statusList?.find((s) => s?.code === realEstate?.statusCode);
            if (status) {
                status.count += 1;
            } else {
                statusList.push({
                    code: realEstate.RealEstateStatus.code,
                    title: realEstate.RealEstateStatus[getNameLanguage()],
                    count: 1,
                });
            }
        });
    ...
    const fetchRealEstates = async () => {
        try {
            ...
            const res = await fetch(
                process.env.NEXT_PUBLIC_EVENTS_URL + `/realEstates/getAllForAuthor/${getUserId()}`,
                {
                    headers: {
    ...
    const toPublish = async (realEstate: any) => {
        try {
...
export default MyRealEstateList;
