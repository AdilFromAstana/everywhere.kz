...
const EventCard = ({ UserLang, data, UserCityId, cardType = 'semi' }: EventCardProps) => {
    return (
        <div
            key={data.id}
            className={cardType === 'semi' ? 'lg:min-w-[20%] lg:max-w-[20%] min-w-[40%] max-w-[40%] my-1' : 'w-full'}
        >
                ...
                <div className="flex flex-col gap-2 w-full cursor-pointer rounded-xl">
                    <div className="relative rounded-xl overflow-hidden md:hover:shadow-xl transition duration-200">
                        <Image
                            ...
                            className={`${
                                cardType === 'semi' ? 'min-h-[238px]' : 'min-h-[290px]'
                            } bg-[#d9d9d9] w-full object-cover lg:min-h-[354px] hover:scale-105 transition duration-200`}
                            src={
                    ...
                    <div>
                        <h6 className="mb-1 md:text-2xl text-sm leading-tight line-clamp-2 font-bold text-[#2F2F38D9] dark:text-white text-ellipsis whitespace-break-spaces overflow-hidden">
                            {data.name}
                        </h6>
                        <h3 className="text-[#00000073] lg:text-sm text-xs dark:text-white flex flex-col">
