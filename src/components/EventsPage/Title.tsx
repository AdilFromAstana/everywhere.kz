...
const Title = ({ cities, selectedCity, title, locale, icon }: TitleProps) => {
        ...
        <div className="relative container z-40 mx-auto">
            <div className="flex lg:gap-4 gap-2 items-center lg:mt-0 mt-6 flex-wrap">
                {icon && !isEmpty(icon) ? (
                    ...
                    <div
                        className={`flex items-center text-black dark:text-white lg:w-16 lg:h-16 ${
                            title.length > 6 ? 'w-10 h-10' : 'w-12 h-12'
                        }`}
                    >
                ...
                <h1 className={`lg:text-4xl ${title.length > 6 ? 'text-base' : 'text-2xl'} dark:text-white font-bold`}>
                    {title}
                </h1>
