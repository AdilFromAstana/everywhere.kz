...
const Header: React.FC<HeaderProps> = ({ locale, selectedCity, cities, langs, selectedLang, token, lookingFor }) => {
    ...
    useEffect(() => {
        ...
        const ViewedRealEstatePhones = getCookie('ViewedRealEstatePhones');
        const ViewedRealEstates = getCookie('ViewedRealEstates');
        ...
        if (isEmpty(ViewedRealEstates)) {
            setCookie('ViewedRealEstates', JSON.stringify([]), {
                maxAge: 60 * 60 * 24 * 1,
            });
        }
        if (isEmpty(ViewedRealEstatePhones)) {
            setCookie('ViewedRealEstatePhones', JSON.stringify([]), {
                maxAge: 60 * 60 * 24 * 1,
            });
        }
                        ...
                        {token ? (
                            <Popover className="relative">
                                ...
                                >
                                    <Popover.Panel className="absolute z-50 -right-8 top-full mt-3 w-screen max-w-xs max-h-96 overflow-auto rounded-2xl bg-white shadow-lg ring-1 ring-gray-900/5">
                                        {[
                                            { href: '/realEstate/create', text: locale.Common.CreateRealEstate },
                                            { href: '/realEstate/list', text: locale.Header.Nav.MyRealEstates },
                            ...
                            <Link href="/auth">{locale.Header.Nav.Enter}</Link>
                        )}
                    </div>
