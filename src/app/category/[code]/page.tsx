...
const CustomCategories = [
    {
        nameKz: 'Концерттер',
        nameEn: 'Concerts',
        nameRu: 'Грандиозные концерты',
        code: 'concerts',
        icon: (
            <svg
                className="lg:w-16 lg:h-16 w-10 h-10 text-[#2F2F38] dark:text-white"
                width="77"
                height="77"
                viewBox="0 0 77 77"
                fill="none"
...
export default async function CategoryPage({ params }: Props) {
    ...
    if (selectedCategory && !isEmpty(selectedCategory)) {
        ...
        let customCategory: any = CustomCategories.find((x) => x.code === selectedCategory.code);

        if (customCategory) {
            switch (UserLang?.toLocaleLowerCase()) {
                case 'kk': {
                    customCategory = { ...customCategory, name: customCategory.nameKz };
                    break;
                }
                case 'en': {
                    customCategory = { ...customCategory, name: customCategory.nameEn };
                    break;
                }
                default: {
                    customCategory = { ...customCategory, name: customCategory.nameRu };
                    break;
                }
            }
        ...
        return (
            <>
                <Title
                    ...
                    title={customCategory?.name ?? selectedCategory.name}
                    cities={cities}
                    selectedCity={selectedCity}
                    locale={locale}
                />
                <HorizontalCalendar />
                <div className="grid lg:grid-cols-5 grid-cols-2 lg:gap-5 gap-3 container mx-auto content-center justify-items-center">
