...
const ModalHeader: React.FC = () => {
    ...
    const { event, sessions, hall } = useAppSelector(({ mainReducer }) => mainReducer);
    ...
    if (isMobile) {
        return (
            <div className="flex items-center justify-between font-medium lg:hidden">
                <button className="h-4 w-4" disabled={activeStepIndex === 0} onClick={prevStep}>
                    <ArrowIcon />
                </button>
                <div className="text-lg">{StepItems()[activeStepIndex].title}</div>
                <button className="h-4 w-4" onClick={cancelOrder}>
                    X
                </button>
            </div>
        );
                    ...
                    return (
                        <div
                            ...
                            className={`font-semibold ${
                                stepItem.activeStepIndex === activeStepIndex ? 'text-[#0590C4]' : 'text-[#1A1A1A]'
                            }`}
                        >
                            {stepItem.title}
                        </div>
