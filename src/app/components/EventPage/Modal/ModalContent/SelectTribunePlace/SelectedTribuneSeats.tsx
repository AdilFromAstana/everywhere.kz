...
const SelectedTribuneSeats: React.FC = () => {
    ...
    const deleteSelectedSeat = (e: any, deleteTicket: any) => {
        const elementOnThePage = document.querySelector(`[key="${deleteTicket.key}"]`) as HTMLElement;
        if (elementOnThePage) {
            elementOnThePage.style.backgroundColor = deleteTicket.serviceColor;
            dispatch(mainReducer.actions.setTickets(tickets.filter((ticket) => ticket.key !== deleteTicket.key)));
        }
    };
    ...
    const getTotalOfServicesFee = (): number => {
        return (getTotalOfServices() / 100) * (event?.serviceFee ?? 1);
    };
    ...
    return (
        <div className="my-4 flex flex-col gap-2">
            <div className="text-lg font-semibold">Кол. билетов: {tickets.length}</div>
            ...
            {!isMobile && (
                <>
                    ...
                    <div className="text-lg font-semibold">
                        <span className="mr-2 text-gray-500">Сервисный сбор {event?.serviceFee}%:</span>
