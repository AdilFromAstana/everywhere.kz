...
const SelectSector = () => {
    const { hall, selectedSession, sector } = useAppSelector(({ mainReducer }) => mainReducer);

    const svgTest = useRef<HTMLDivElement | null>(null);
    const [currentSector, setCurrentSector] = useState<ICurrentSector | null>(null);
    ...
    const sectorClick = async (e: any) => {
        ...
        if (sectorAlreadySelected) {
            ...
            await Events()
                .get(`Sessions/places?sessionId=${selectedSession?.id}&sectorId=${sessionSectorId}`)
                .then(({ data }) => {
                    dispatch(
                    ...
                    dispatch(mainReducer.actions.setCustomServiceGroups(data.customServiceGroups));
    ...
    const showTooltipOnSector = (event: MouseEvent) => {
        ...
        const tooltip = document.querySelector<HTMLElement>('#tooltip-sector');

        if (targetSeat && tooltip) {
            ...
            const needSector = hall!.sectors.find((el: any) => el.id === sessionSectorId);
    ...
    const getScheme = (initialScheme: string) => {
        ...
        const parsedScheme = new DOMParser().parseFromString(newScheme, 'text/html');
        const firstChild = parsedScheme.body.firstChild;
        const testScheme = firstChild ? (firstChild.cloneNode(true) as HTMLElement) : null;
        if (testScheme) {
            testScheme.querySelectorAll('path').forEach((element) => {
                hall!.sectors.forEach((sector: any) => {
                    const isSectorRegistred = sector.sectorId === element.getAttribute('sector-id');
                    ...
                    <TransformWrapper initialScale={1} initialPositionX={0} initialPositionY={0}>
                        {({ zoomIn, zoomOut, resetTransform, ...rest }) => (
                            <React.Fragment>
                                ...
                                <TransformComponent wrapperClass="SvgWrapper" contentStyle={{ width: '100%' }}>
                                    <div
                                        ref={svgTest}
                                        style={{ width: '100%' }}
                    ...
                    <div id="tooltip-sector" className="tooltip-open">
                        <div id="tooltip_select">
                            <div className="flex flex-col">
                                {isEmpty(currentSector?.nameRu) ? null : <span>{currentSector?.nameRu}</span>}
                                <span>
                                    sectorType : {currentSector?.typeId === 2 ? 'tribuneSeat' : 'standingPlace'}
                                </span>
                                <span>
                                    {currentSector?.typeId === 2 ? 'Доступных мест' : 'balanceCount'}:{' '}
