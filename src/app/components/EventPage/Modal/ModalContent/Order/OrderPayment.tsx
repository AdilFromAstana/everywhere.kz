...
const OrderPayment: React.FC = () => {
    ...
    const [refreshTimeExpired, setRefreshTimeExpired] = useState(moment().format());
                ...
                .then(({ data }: any) => {
                    switch (data.statusId) {
                        ...
                        case OrderStatus.Canceled: {
                            dispatch(mainReducer.actions.clearOrderInfo());
                            dispatch(modalReducer.actions.resetPaymentStepIndex());
        ...
        tickets.forEach((ticket) => {
            if (ticket.ticketCount > 0) {
                services.push({
                    ...
                    trubuneSeatId: sector?.scheme === 'typeIdOne' ? null : ticket.trubuneSeatId,
                    standingPlaceId: sector?.scheme === 'typeIdOne' ? serviceGroups?.id : null,
    ...
    const paymentWithCloudPayments = async () => {
        const amountWithService = totalAmount + (totalAmount / 100) * (isEmpty(event) ? event!.serviceFee : 1);
        const services: any = [];
        tickets.forEach((ticket) => {
            if (ticket.ticketCount > 0) {
                services.push({
    ...
    if (isLoading) {
        return (
            <div className="flex h-full flex-col items-center justify-center">
                ...
                <div className="text-4xl">Загрузка</div>
            </div>
        );
    } else {
        if (error.isError) {
          ...
          /> */}
                    <div className="OrderInfoComponent mb-2 flex flex-col items-center justify-center gap-2 text-xl">
                        ...
                        <span className="Status">
                            Статус заказа: {currentOrderData?.statusId ? OrderStatus[currentOrderData?.statusId] : ''}
                        </span>
                        <Countdown
