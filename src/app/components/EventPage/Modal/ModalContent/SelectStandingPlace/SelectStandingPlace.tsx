...
const ServiceComponent: React.FC<{
  ...
  const serviceCountInput = isMobile
    ? 'h-8 w-24 rounded-xl border-2'
    : 'h-10 w-36 rounded-lg border';

  const ticket = tickets?.find(
    (ticket) => ticket?.sessionServiceId === service.sessionServiceId
  );
  ...
  useEffect(() => {
    ...
    let isLeftPlacesCountMoreThanFive = leftPlacesCount > 10;
      ...
      <div className={`${isMobile ? 'font-medium' : 'text-xl'} mb-5`}>
        ...
        <div className={`mt-2  ${isMobile ? 'flex justify-between' : ''}`}>
          <div className='flex h-8 space-x-2.5'>
            ...
            <div
              className={`${serviceCountInput} flex items-center justify-start border-[#BCBCBC] pl-2`}
            >
...
const SelectStandingPlace: React.FC = () => {
  ...
  const { tickets, serviceGroups, customServiceGroups, event, totalAmount } =
  ...
  if (isMobile) {
    return (
      <div className='h-full'>
        {serviceGroups.sessionServices.map(
          (sessionService: IServiceForOrder) => (
            <ServiceComponent
              service={sessionService}
              leftPlacesCount={serviceGroups?.balanceCount}
    ...
    return (
      <>
        <div className='mb-6 text-2xl font-semibold'>
          Массовые катания в ледовом дворце “Алау”
        </div>
                      ...
                      return (
                        <ServiceComponent
                          ...
                          increment={() => {
                            ...
                            dispatch(
                              mainReducer.actions.setTickets(ticketCountArray)
                            );
                          }}
                          dicrement={() => {
                            const ticketCountArray = tickets.map((ticket) => {
          ...
          <div className='text-xl'>
            ...
            <div className='Total'>
              ...
              <span className='font-bold'>
                {(
                  totalAmount +
                  (totalAmount / 100) * (event?.serviceFee ?? 1)
                ).toFixed(2)}
                ₸
              </span>
            </div>
